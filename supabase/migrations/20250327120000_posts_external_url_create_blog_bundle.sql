-- RSS 원문 URL로 중복 방지 + create_blog_bundle 에 external_url 반영

alter table public.posts
  add column if not exists external_url text;

comment on column public.posts.external_url is
  'RSS 등 외부 원문 링크. 같은 URL은 한 번만 적재(유니크).';

create unique index if not exists posts_external_url_unique
...
begin
  ...
  insert into public.posts (title, content, slug, excerpt, published, category_id, source, external_url)
  values (
    payload->>'title',
...
comment on function public.create_blog_bundle(jsonb) is
  'JSON payload: title, content, slug, excerpt?, published?, source?, external_url?, category?:{name,slug}, tags?:[{name,slug}]. Returns new post id.';
